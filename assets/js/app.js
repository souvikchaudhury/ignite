...
var igFinance = (function () {
    var helpers = (function () {
        ...
        function _setpriceChartinterval(interval) {
            $("#priceChart").igDataChart("option", "axes", [{
                name: "xAxis2",
                type: "categoryX",
                interval: interval
            }]);
        ...
        function _itemscan (item) {
            ...
            var x = '';
            //var chartWidth = $("#priceChart").igDataChart("option", "windowRect").width.toFixed(2);
            //console.log(chartWidth);

            /*switch (chartWidth) {
                case (chartWidth >= 0.05 && chartWidth <= 0.20):
                    console.log("0.05, 0.20");
                    _setpriceChartinterval(0);
                    break;

                case (chartWidth >= 0.20 && chartWidth <= 0.40):
                    console.log("0.20, 0.40");
                    _setpriceChartinterval(10);
                    break;

                case (chartWidth >= 0.40 && chartWidth <= 0.60):
                    _setpriceChartinterval(10);
                    break;

                case (chartWidth >= 0.60 && chartWidth <= 0.80):
        ...
        function _formatLabel(item) {
            ...
            //console.log(chartWidth);
            if (chartWidth >= 0.05 && chartWidth <= 0.20) {
                console.log("0.05, 0.20");
                setInterval = 3;
            } else if (chartWidth >= 0.20 && chartWidth <= 0.40) {
                console.log("0.20, 0.40");
                setInterval = 10;
            } else if (chartWidth >= 0.40 && chartWidth <= 0.60) {
                setInterval = 20;
            } else if (chartWidth >= 0.60 && chartWidth <= 0.80) {
                console.log("0.60, 0.80");
                setInterval = 40;
            ...
            _setpriceChartinterval(setInterval);
        ...
        init: function () {
            ...
            $.get('stockScan.php', {symbolname : window.selectedSymbol}).done(function (data) {
                ...
                if (returnData.success == true) {
                    ...
                    //var returnData = JSON.parse(data);
                    window.scanResultDates = returnData.message.scanDate;
                    console.log(window.scanResultDates);
                }                
...
//        showPinButton: true,
    ...
    $("#portfolioFormdialog").igDialog("close");
    $('.portfolio_buy').on({
        click: function () {
            // Open the igDialog
            $("#portfolioFormdialog").igDialog("open");
        }
    });
