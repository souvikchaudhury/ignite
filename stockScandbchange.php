...
//     while($scanningRow = mysqli_fetch_array($scanningresults)) {
      ...
      if($scanningRowdd[0]>=1){
    	...
    	// $tradetableresults = $scanningRow['tradetableresults'];
    	// // $scanningRow
     ...
     //   	echo $sql_data_update = "UPDATE stockscaning SET tradetableresults= '".$tradetableresults."' WHERE id='".$scanningRow['id']."'";
     //   	$date_update_result = mysqli_query($conn,$sql_data_update);
     //    var_dump($date_update_result);
   		// die();
    // }
...
$subject = 'Scanning & Performance '.$_POST['symbol'].date("F j, Y, g:i a");
$message = 'Run Timed '.date("F j, Y, g:i a");
